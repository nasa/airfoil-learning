...
def create_torch_geometric_train_test_data(scraped_filename,varying_nodes=False,use_node_labels=True,processed_path='processed',scale_y=False,scale_cp=False, scale_y_indiv=False,scale_cp_indiv=False,include_cond_in_node_features=True):
    '''
    ...
    for a in trange(len(df),desc='reading data into list'):
        ...
        if (scale_y_indiv): # Scale the y's individually            
            for i in range(len(y)):
                y[i] = scalers_y[i].transform(y[i].reshape(-1,1))[0][0]
        else:
            y = scalers['y'].transform(y.reshape(-1,1))
        ...
        for p in range(len(airfoil['polars'])):            
            ...
            if varying_nodes:
                ...
                # x = scalers['x'].transform(x.reshape(-1,1))
                if (scale_y):
                    if (scale_y_indiv): # Scale the y's individually
                        for i in range(len(y)):
                            y[i] = scalers_y[i].transform(y[i].reshape(-1,1))
                    else:
                        y = scalers['y'].transform(y.reshape(-1,1))
            ...
            if (scale_y):
                ...
                Cl = polar['Cl'].reshape(-1,1)
                Cd = polar['Cd'].reshape(-1,1)
                Cdp = polar['Cdp'].reshape(-1,1)
                Cm = polar['Cm'].reshape(-1,1)
            
            if (use_node_labels):
                node_labels = np.concatenate(( Cp_ss, np.flip(Cp_ps[1:-1]) ))
                if (scale_cp):
                    if (scale_cp_indiv): # Scale the Cp's individually
                        for i in range(len(node_labels)):
                            node_labels[i] = scalers_cp[i].transform(node_labels[i].reshape(-1,1))[0][0]
                    else:
            ...
            if (include_cond_in_node_features):
                data_x = torch.as_tensor(np.hstack([ x,y, alpha*ones, Reynolds*ones, Ncrit*ones]), dtype=torch.float32)
                conditions=None
            else:
                data_x = torch.as_tensor(np.hstack([ x,y]), dtype=torch.float32)
            ...
            d = Airfoil(x=data_x,conditions=conditions,edge_index=edge_index,edge_attr=edge_attr,y=data_y,node_labels=node_labels)
...
if __name__ == "__main__":
    ...
    create_torch_geometric_train_test_data('scraped_airfoils_cp_clean.gz',varying_nodes=False,use_node_labels=True,processed_path='processed_non_varying_node_labels',scale_y=False,scale_cp=False,include_cond_in_node_features=False)
    ...
    # create_train_test_data('scraped_airfoils_cp.gz',varying_nodes=False,use_node_labels=False,processed_path='processed_non_varying')
