...
def create_torch_geometric_train_test_data(scraped_filename,varying_nodes=False,processed_path='processed'):
    ...
    for a in trange(len(df),desc='reading data into list'):
        ...
        # y_pseudo = scalers['y'].transform(y.reshape(-1,1))             # spline conv uses pseudo coordinates 0 to 1 for both x and y, use this as the node features
        ...
        for p in range(len(airfoil['polars'])):            
            ...
            data_x = torch.as_tensor(np.hstack([alpha*ones, Reynolds*ones, Ncrit*ones]), dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    create_torch_geometric_train_test_data('scraped_airfoils_cp_clean.gz',varying_nodes=False,processed_path='processed_non_varying_node_labels_v02')
