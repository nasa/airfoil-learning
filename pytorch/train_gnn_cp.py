...
def train_one_epoch(train_dl:DataLoader,optimizer:torch.optim.Optimizer, model:torch.nn.Module,output_channels:int,scheduler=None):
    ...
    for _, data in enumerate(pbar):        
        try:
            ...
            #     param.grad = None
            out = model(data)
            target =torch.cat([data.y.reshape(-1,1),data.node_labels],axis=0).reshape(data.num_graphs,output_channels)
...
if __name__=="__main__":
    ...
    with open('settings.json', 'r') as infile:          # Loads the settings for the graph neural network 
        ...
        dataset_type = 'gnn'
        gnn_networks = [a for a in args['networks'] if a['type'] == dataset_type and a['train']['predict_cp']== True]
        for args in gnn_networks:
            ...
            train_args['output_size'] = data_params['output_size'] + data_params['node_labels']
            ...
            for epoch in trange(start_epoch, train_args['epochs']+1):                      
                loss = train_one_epoch(train_dl,optimizer,model,train_args['output_size'],scheduler)
                if epoch % train_args['validation_epoch'] == 0:
                    val_loss = evaluate(test_dl,model,train_args['output_size'])
